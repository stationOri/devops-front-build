...
function RestRun({ restId }) {
  ...
  const GetOpenTime = async (restId) => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_API_URI}/api/opentime/${restId}`);
  ...
  const updateOpenTimes = async () => {
    ...
    try {
      for (const day of checkedDays) {
        ...
        await axios.put(`${process.env.REACT_APP_API_URI}/api/opentime/rest/${restId}/day/${day}`, data);
