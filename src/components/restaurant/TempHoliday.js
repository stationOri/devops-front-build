...
function TempHoliday({ restId }) {
  ...
  useEffect(() => {
    const fetchTempHolidays = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_API_URI}/api/restaurants/rest-temp-holiday/${restId}`);
  ...
  const handleDelete = async (holidayId) => {
    ...
    try {
      await axios.delete(`${process.env.REACT_APP_API_URI}/api/restaurants/rest-temp-holiday/${holidayId}`);
      const response = await axios.get(`${process.env.REACT_APP_API_URI}/api/restaurants/rest-temp-holiday/${restId}`);
      const data = Array.isArray(response.data) ? response.data : [];
      setTempHolidays(data);
    } catch (error) {
