...
const Restaurants = ({ userId, onCardClick }) => {
    ...
    try {
      if (page <= (maxPages || Infinity)) {
        ...
        const response = await fetch(
          `http://https://waitmate.shop/api:8080/restaurants/page/${page}`
        );
        if (!response.ok) {
  ...
  const fetchRestaurantByName = async (name) => {
    try {
      const response = await fetch(
        `http://https://waitmate.shop/api:8080/restaurants/name/${name}`
      );
      if (!response.ok) {
  ...
  const fetchFavorites = async () => {
    try {
      const response = await fetch(`http://https://waitmate.shop/api:8080/favorite/${userId}`);
      if (!response.ok) {
  ...
  const fetchKeywords = async () => {
    try {
      const response = await fetch(`http://https://waitmate.shop/api:8080/keywords`);
      if (!response.ok) {
  ...
  const toggleFavorite = async (restId, isFavorite) => {
    try {
      ...
      if (existingFavorite) {
        const response = await fetch(
          `http://https://waitmate.shop/api:8080/favorite/${existingFavorite.favoriteId}`,
          {
            method: "DELETE",
            headers: {
